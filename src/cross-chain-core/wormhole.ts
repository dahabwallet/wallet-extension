...
import { Signer, sendAndConfirmTransaction, Keypair } from "@solana/web3.js";
...
async function get_reciever_address(public_key_sol: String): Promise<string> {
  ...
  const weth_sol_addr = new web3_sol.PublicKey(
...
async function redeem(tx: ethers.ContractReceipt) {
  ...
  const user_sol_pubkey = new web3_sol.PublicKey("SOL_PUBLIC_KEY_STRING");
  let secretKey = bs58.decode("SOL_PRIVATE_KEY_STRING");

  const signer: Signer = {
    publicKey: user_sol_pubkey,
    secretKey,
  };
  const signer_factory = await signTransactionFactory(signer);

  await postVaaSolanaWithRetry(
    solana_node,
    signer_factory,
    SOL_BRIDGE_ADDRESS,
    "SOL_PUBLIC_KEY_STRING",
...
async function swap_eth_to_sol(to_transfer: BigInteger) {
  let signer = new ethers.Wallet("ETH_PRIVATE_KEY_STRING");
  ...
  const calculate_token_account = await get_reciever_address(
    "SOLANA_PUBLIC_KEY_STRING"
  );
