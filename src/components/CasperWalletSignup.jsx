...
class CasperWalletSignup extends Component {
	constructor() {
		super();
		this.state = {value: ''};
	...
	*/	
	handleChange(event) {
	  ...
	  handleSubmit(event) {
		...
		this.createWallet(this.state.value);
	  }
	    render() { 
			return (
                <Fragment>
	...
	async createWallet(keyName){
		console.log("creataing wallet after mounting the component");
			...
			this.storeKey(keyName);
		}

		async storeKey (keyName){
			...
			try {
				...
				Name: this.getSecretIDForKey(keyName),
				SecretString: JSON.stringify({[this.publicKeyInPem]: this.privateKeyInPem,}),
				}).promise();
...
export default CasperWalletSignup;
